...
def show_box_on_views(classes, images: Tuple[Image.Image, ...],
    ...
    for idx, img in enumerate(images):
        ...
        if aug_matrix is not None:
            extra = {
                "extra_rots": aug_matrix[idx][:3, :3],
                "extra_trans": aug_matrix[idx][:3, 3],
            }
        else:
            extra = {}
        img_out = visualize_camera(
            None, image=image, bboxes=bboxes_trans, labels=labels,
            transform=trans, classes=classes, thickness=1, **extra,
        )
...
def prepare_ckpt(path, download=False):
    # deleted funtion, please prepare you ckpt
